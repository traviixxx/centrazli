#!/usr/bin/env python3
"""
SOL HVN/LVN zone-touch trader (SIM or LIVE Binance Futures).
...
LIVE: export SOL_LIVE_TRADE=true BINANCE_API_KEY=... BINANCE_ED25519_PRIVATE_KEY_PATH=...
      export SOL_ORDER_USDT=1000   # margin (Binance "Cost") USDT
      export SOL_LEVERAGE=40       # position notional ≈ 1000 × 40 = 40,000 USDT
      export SOL_ENTRY_ORDER_TYPE=LIMIT
      export SOL_LIMIT_ENTRY_OFFSET=0
      export SOL_ENTRY_ORDER_TIMEOUT_SEC=90
      # chỉ notional 1000 (không × lev): export SOL_SIZING_MODE=notional
"""
...
# CONFIG
# =========================
SYMBOL = os.getenv("SOL_SYMBOL", "SOLUSDT")
FUTURES_BASE = "https://fapi.binance.com"

ZONE_TOUCH_TOLERANCE = float(os.getenv("ZONE_TOUCH_TOLERANCE", "0.03"))
TP_LVN_POINTS = float(os.getenv("TP_LVN_POINTS", "0.13"))
TP_HVN_POINTS = float(os.getenv("TP_HVN_POINTS", "0.23"))
SL_POINTS = float(os.getenv("SL_POINTS", "0.23"))
...
STRICT_EDGE_ONLY = env_bool("SOL_STRICT_EDGE_ONLY", True)
ENABLE_MAX_INVERSE = env_bool("SOL_ENABLE_MAX_INVERSE", True)
ENABLE_REJECTION_FOLLOW_TREND = env_bool("SOL_ENABLE_REJECTION_FOLLOW_TREND", True)
ENABLE_SHORT_BEARISH_4H_MIN_EDGE = env_bool("SOL_ENABLE_SHORT_BEARISH_4H_MIN_EDGE", True)
MAX_RETEST_IMPULSE_POINTS = float(os.getenv("SOL_MAX_RETEST_IMPULSE_POINTS", "0.6"))
RETEST_LOOKBACK_CANDLES = int(os.getenv("SOL_RETEST_LOOKBACK_CANDLES", "8"))
REJECTION_LOOKBACK_CANDLES = int(os.getenv("SOL_REJECTION_LOOKBACK_CANDLES", "3"))
REJECTION_WICK_RATIO = float(os.getenv("SOL_REJECTION_WICK_RATIO", "1.2"))

# =========================
# MARKET CONTEXT ROUTER
# =========================
# Context first, strategy second:
# - RANGE_MEAN_REVERSION allows HVN/LVN scalp behavior such as max-inverse.
...
ACCEPTANCE_CANDLES = int(os.getenv("SOL_ACCEPTANCE_CANDLES", "2"))
...
# Breakout/reclaim continuation edge. Default ON because it directly fixes the
# SOL failure mode where HVN max flips from resistance into support.
ENABLE_LONG_BULLISH_4H_MAX_EDGE = env_bool("SOL_ENABLE_LONG_BULLISH_4H_MAX_EDGE", True)
LONG_RETEST_REQUIRE_ACCEPTANCE = env_bool("SOL_LONG_RETEST_REQUIRE_ACCEPTANCE", True)
LONG_BULLISH_4H_MAX_ALLOWED_TF = os.getenv("SOL_LONG_BULLISH_4H_MAX_ALLOWED_TF", "4h").strip().lower()
LONG_BULLISH_4H_MAX_MAX_PULLBACK_POINTS = float(os.getenv("SOL_LONG_BULLISH_4H_MAX_MAX_PULLBACK_POINTS", "0.45"))

# Do not re-enter the same side/zone immediately after a stop loss. This prevents
# repeated shorting of the same broken HVN max during breakout acceptance.
ZONE_COOLDOWN_AFTER_SL_SEC = float(os.getenv("SOL_ZONE_COOLDOWN_AFTER_SL_SEC", "3600"))
...
# Old behavior: restarting the script gives a fresh session and lets it run again.
# Set SOL_RESET_LOSS_STREAK_ON_START=false if you want the stricter v8 behavior
# that restores the current loss streak from trade_history and blocks after restart.
RESET_LOSS_STREAK_ON_START = env_bool("SOL_RESET_LOSS_STREAK_ON_START", True)
STARTING_BALANCE = float(os.getenv("SIM_STARTING_BALANCE", "10000.0"))

STATE_PATH = os.path.join(
    _SCRIPT_DIR,
    os.getenv("SOL_HVN_LVN_STATE", "SOL_hvn_lvn_trade_state.json"),
)
TRADE_LOG_PATH = os.path.join(
    _SCRIPT_DIR,
    os.getenv("SOL_HVN_LVN_LOG", "SOL_hvn_lvn_trades.jsonl"),
)

PRICE_DECIMALS = int(os.getenv("PRICE_DECIMALS", "3"))

LIVE_TRADE = env_bool("SOL_LIVE_TRADE", False)
...
ORDER_USDT = float(os.getenv("SOL_ORDER_USDT", os.getenv("SOL_MARGIN_USDT", "1000")))
LEVERAGE = int(os.getenv("SOL_LEVERAGE", "40"))
# margin (default): notional = ORDER_USDT * LEVERAGE | notional: ORDER_USDT only
SIZING_MODE = os.getenv("SOL_SIZING_MODE", "margin").strip().lower()
_ORDER_QTY_RAW = os.getenv("SOL_ORDER_QTY", "").strip()
...
ENTRY_ORDER_TYPE = os.getenv("SOL_ENTRY_ORDER_TYPE", "LIMIT").strip().upper()
LIMIT_ENTRY_OFFSET = float(os.getenv("SOL_LIMIT_ENTRY_OFFSET", "0"))
ENTRY_ORDER_TIMEOUT_SEC = float(os.getenv("SOL_ENTRY_ORDER_TIMEOUT_SEC", "90"))
...
BRACKET_MODE = os.getenv("SOL_BRACKET_MODE", "AUTO").strip().upper()
BRACKET_CLOSE_POSITION = env_bool("SOL_BRACKET_CLOSE_POSITION", True)
BRACKET_WORKING_TYPE = os.getenv("SOL_BRACKET_WORKING_TYPE", "MARK_PRICE").strip().upper()
POSITION_SIDE_MODE = os.getenv("BINANCE_POSITION_SIDE_MODE", "AUTO").strip().upper()  # AUTO | ONEWAY | HEDGE
BRACKET_RETRY_ATTEMPTS = max(1, int(os.getenv("SOL_BRACKET_RETRY", "1")))
BRACKET_RETRY_SLEEP_SEC = float(os.getenv("SOL_BRACKET_RETRY_SLEEP_SEC", os.getenv("SOL_BRACKET_RETRY_SEC", "3")))
BRACKET_RETRY_SEC = float(os.getenv("SOL_BRACKET_RETRY_SEC", str(BRACKET_RETRY_SLEEP_SEC)))
POSITION_STATUS_SEC = float(os.getenv("SOL_POSITION_STATUS_SEC", str(PRICE_POLL_SEC)))
# Binance USD-M now requires conditional TP/SL via Algo Order API on some accounts/API versions.
BRACKET_API = os.getenv("SOL_BRACKET_API", "ALGO").strip().upper()  # ALGO or ORDER
...
def _closed_context_candles(symbol, tf=None, limit=None):
    """Return recent closed OHLC candles; never use the currently forming candle for context."""
    tf = tf or CONTEXT_TF
    limit = limit or max(CONTEXT_LOOKBACK_CANDLES + 3, LATE_IMPULSE_LOOKBACK_CANDLES + 3, 12)
    candles = get_klines_ohlc(symbol, tf, limit)
    if len(candles) < 2:
        return []
    return candles[:-1]


def _structure_flags(candles):
    """Lightweight HH/HL and LH/LL structure detector from closed candles."""
    if len(candles) < 4:
        return {
            "higher_high": False,
            "higher_low": False,
...
def classify_market_context(price, trend, trend_15m, zone, level_name=None):
    """
    Context router:
    The script must decide whether an HVN/LVN level is acting as resistance,
    support, or a broken/accepted level before allowing a strategy rule.
    """
    if not CONTEXT_ROUTER_ENABLED:
        return {
            "context": MARKET_CONTEXT_LEGACY,
            "reason": "context_router_disabled",
    ...
    try:
        ...
        return {
            "context": MARKET_CONTEXT_UNKNOWN,
            "reason": "invalid_zone_bounds",
            "accepted_above": False,
            "accepted_below": False,
        }

    lookback = max(CONTEXT_LOOKBACK_CANDLES, ACCEPTANCE_CANDLES + 2, LATE_IMPULSE_LOOKBACK_CANDLES + 2, 8)
    closed = _closed_context_candles(SYMBOL, CONTEXT_TF, lookback + 3)
    if len(closed) < max(ACCEPTANCE_CANDLES, 2):
        return {
            "context": MARKET_CONTEXT_UNKNOWN,
            "reason": "not_enough_context_candles",
            "accepted_above": False,
            "accepted_below": False,
        }

    recent = closed[-lookback:]
    acceptance_window = recent[-ACCEPTANCE_CANDLES:]
    closes_above_high = sum(
        1 for c in acceptance_window if float(c["close"]) > zone_high + ACCEPTANCE_BUFFER
    )
    closes_below_low = sum(
        1 for c in acceptance_window if float(c["close"]) < zone_low - ACCEPTANCE_BUFFER
    )
    accepted_above = closes_above_high >= ACCEPTANCE_CANDLES
    accepted_below = closes_below_low >= ACCEPTANCE_CANDLES

    structure = _structure_flags(recent)
    trend_15m = float(trend_15m or 0.0)

    impulse_window = recent[-max(2, min(LATE_IMPULSE_LOOKBACK_CANDLES, len(recent))):]
    impulse_points = abs(float(impulse_window[-1]["close"]) - float(impulse_window[0]["close"]))
    impulse_up = float(impulse_window[-1]["close"]) > float(impulse_window[0]["close"])
    impulse_down = float(impulse_window[-1]["close"]) < float(impulse_window[0]["close"])

    meta = {
        "accepted_above": accepted_above,
        "accepted_below": accepted_below,
        "closes_above_high": closes_above_high,
        "closes_below_low": closes_below_low,
        "zone_low": zone_low,
        "zone_high": zone_high,
        "acceptance_buffer": ACCEPTANCE_BUFFER,
        "acceptance_candles": ACCEPTANCE_CANDLES,
        "context_tf": CONTEXT_TF,
        "impulse_points": round(impulse_points, PRICE_DECIMALS),
        **structure,
    }

    if accepted_above and trend_15m > 0 and (trend == "bullish" or structure["higher_low"] or structure["higher_high"]):
        meta.update({
            "context": MARKET_CONTEXT_BREAKOUT_UP,
            "reason": (
                f"accepted_above_zone_high_{zone_high:.{PRICE_DECIMALS}f}_"
                f"{closes_above_high}/{ACCEPTANCE_CANDLES}c;trend_15m_positive"
            ),
        })
        return meta

    if accepted_below and trend_15m < 0 and (trend == "bearish" or structure["lower_high"] or structure["lower_low"]):
        meta.update({
            "context": MARKET_CONTEXT_BREAKDOWN_DOWN,
            "reason": (
                f"accepted_below_zone_low_{zone_low:.{PRICE_DECIMALS}f}_"
                f"{closes_below_low}/{ACCEPTANCE_CANDLES}c;trend_15m_negative"
            ),
        })
        return meta

    # Late impulse is evaluated after acceptance. A breakout can be valid but too late to chase;
    # the dedicated retest edge will still require pullback not to be too large.
    if impulse_points > LATE_IMPULSE_POINTS:
        side = "up" if impulse_up else ("down" if impulse_down else "flat")
        meta.update({
            "context": MARKET_CONTEXT_LATE_IMPULSE,
            "reason": f"late_impulse_{side}_{impulse_points:.3f}pt_gt_{LATE_IMPULSE_POINTS}",
        })
        return meta
...
def retest_from_above(level_price, price, lookback=None):
    ...
    lookback = lookback or RETEST_LOOKBACK_CANDLES
    closes, _ = get_klines(SYMBOL, CONTEXT_TF, max(lookback + 2, 10))
    if len(closes) < 3:
        return False, "not_enough_context_data"

    level = float(level_price)
    recent_closed = closes[:-1][-lookback:]
    was_above = any(c > level + ACCEPTANCE_BUFFER for c in recent_closed)
    ...
    if LONG_RETEST_REQUIRE_ACCEPTANCE and last_closed < level - ACCEPTANCE_BUFFER:
    ...
    if pullback_points > LONG_BULLISH_4H_MAX_MAX_PULLBACK_POINTS:
        return False, (
            f"late_pullback_{pullback_points:.3f}pt_gt_"
            f"{LONG_BULLISH_4H_MAX_MAX_PULLBACK_POINTS}"
        )

    return True, f"retest_from_above_pullback_{pullback_points:.3f}pt"


def is_long_bullish_4h_max_candidate(price, trend, trend_15m, trend_side, zone_kind, zone, level_name, level_price):
    """Allow a broken HVN max to flip into support instead of shorting it blindly."""
    if not ENABLE_LONG_BULLISH_4H_MAX_EDGE:
        return False, "long_bullish_4h_max_edge_disabled"
    if zone_kind != "hvn":
        return False, "reject_long_max_not_hvn"
    if str(zone.get("timeframe", "")).lower() != LONG_BULLISH_4H_MAX_ALLOWED_TF:
        return False, "reject_long_max_not_allowed_tf"
    if level_name != "max":
        return False, "reject_long_not_max"
    if trend_side != "LONG" and float(trend_15m or 0.0) <= 0:
        return False, "reject_long_trend_side_not_long"

    ok, reason = retest_from_above(level_price, price)
    if not ok:
        return False, reason
    return True, reason


def signal_zone_key(signal_or_trade):
    """Stable key for same-side same-zone cooldown."""
    try:
        level_price = float(signal_or_trade.get("level_price"))
    except (TypeError, ValueError):
        level_price = 0.0

    return "|".join([
        str(signal_or_trade.get("side", "")).upper(),
        str(signal_or_trade.get("zone_kind", "")).lower(),
        str(signal_or_trade.get("zone_timeframe") or signal_or_trade.get("zone", {}).get("timeframe", "")).lower(),
        str(signal_or_trade.get("level_name", "")).lower(),
        f"{level_price:.{PRICE_DECIMALS}f}",
    ])


def _parse_trade_time_to_epoch(value):
    if not value:
        return None
    for fmt in ("%Y-%m-%d %H:%M:%S", "%Y-%m-%dT%H:%M:%S"):
        try:
            return time.mktime(time.strptime(str(value), fmt))
        except Exception:
            continue
    return None


def recent_zone_loss_count(signal, now=None):
    if not ZONE_COOLDOWN_ENABLED or ZONE_COOLDOWN_AFTER_SL_SEC <= 0 or MAX_ZONE_SL_COUNT <= 0:
        return 0
    now = now or time.time()
    key = signal_zone_key(signal)
    count = 0
    for trade in reversed(TRADE_HISTORY):
        closed_epoch = _parse_trade_time_to_epoch(trade.get("closed_ts"))
        if closed_epoch is None:
            continue
        age = now - closed_epoch
        if age > ZONE_COOLDOWN_AFTER_SL_SEC:
            break
        if trade.get("outcome") == "LOSS" and signal_zone_key(trade) == key:
            count += 1
    return count


def zone_cooldown_allows(signal):
    losses = recent_zone_loss_count(signal)
    if losses >= MAX_ZONE_SL_COUNT:
        return False, (
            f"reject_zone_cooldown_after_{losses}_sl_"
            f"{int(ZONE_COOLDOWN_AFTER_SL_SEC)}s_key={signal_zone_key(signal)}"
        )
...
def resolve_side(price, trend, trend_15m, zone_kind, zone, level_name, level_price):
    ...
    # Priority: if HVN max has flipped into support, use bullish retest/reclaim
    # instead of blindly applying max-inverse.
    long_ok, long_reason = is_long_bullish_4h_max_candidate(
        price, trend, trend_15m, trend_side, zone_kind, zone, level_name, level_price
    )
    if long_ok:
        meta["long_retest_reason"] = long_reason
        return "LONG", trend_side, False, "long_bullish_4h_max_retest", meta

    # Legacy max rule: vào tại max theo hướng nghịch đảo; nếu có rejection thì đi theo hướng thuận.
    # It will still be blocked later if the context router says the zone has breakout acceptance.
    if ENABLE_MAX_INVERSE and level_name == "max":
        ...
        if rejection_confirmed:
            return trend_side, trend_side, True, "max_rejection_follow_trend", meta
        return opposite_side(trend_side), trend_side, False, "max_inverse_no_rejection", meta

    return trend_side, trend_side, rejection_confirmed, strategy_rule, meta
...
def validate_strategy_signal(signal, price):
    ...
    rule = signal.get("strategy_rule")

    cooldown_ok, cooldown_reason = zone_cooldown_allows(signal)
    if not cooldown_ok:
    ...
    if rule == "long_bullish_4h_max_retest":
        if CONTEXT_ROUTER_ENABLED and context != MARKET_CONTEXT_BREAKOUT_UP:
            return False, f"reject_long_max_retest_context_{context}"
        signal["side"] = "LONG"
    ...
    if level_name == "max" and ENABLE_MAX_INVERSE:
        ...
        if CONTEXT_ROUTER_ENABLED and MAX_INVERSE_REQUIRE_RANGE_CONTEXT:
            if context != MARKET_CONTEXT_RANGE:
                return False, f"reject_max_inverse_context_{context}"

        signal["gate_reason"] = signal.get("strategy_rule", "max_rule")
        return True, signal["gate_reason"]

    # Keep audited bearish retest edge: 4h SHORT min only.
    if ENABLE_SHORT_BEARISH_4H_MIN_EDGE and side == "SHORT":
        if CONTEXT_ROUTER_ENABLED and context not in (
            MARKET_CONTEXT_BREAKDOWN_DOWN,
            MARKET_CONTEXT_RANGE,
            MARKET_CONTEXT_LEGACY,
            MARKET_CONTEXT_UNKNOWN,
        ):
            return False, f"reject_short_min_context_{context}"
        if trend != "bearish":
    ...
    if STRICT_EDGE_ONLY:
        return False, f"reject_not_in_enabled_edge_context_{context}"

    signal["gate_reason"] = f"legacy_allowed_context_{context}"
    return True, signal["gate_reason"]
...
def bracket_levels(entry, side, zone_kind, sl_points=None, tp_points=None):
    """SL/TP từ entry thực (giá SOL, đơn vị USD)."""
    entry = float(entry)
    side = side.upper()
...
def build_touch_signal(price, trend, trend_15m, zone_kind, zone, level_name, level_price):
    side, trend_side, rejection_confirmed, strategy_rule, extra_meta = resolve_side(
        price, trend, trend_15m, zone_kind, zone, level_name, level_price
    )
    context_meta = classify_market_context(price, trend, trend_15m, zone, level_name)
    entry = float(price)
...
def trade_base(signal, price, cycle_ts):
    return {
        ...
        "context_reason": signal.get("context_reason"),
        "context_meta": signal.get("context_meta"),
...
def open_sim_trade(signal, price, cycle_ts):
    ...
    print(
        ...
        f"trend={signal['trend']} | context={signal.get('market_context')} | "
        f"rule={signal.get('strategy_rule')} | gate={signal.get('gate_reason')}"
    )
    return ACTIVE_TRADE
...
def open_live_trade(signal, price, cycle_ts):
    ...
    print(
        ...
        f"{signal['zone_kind'].upper()} {signal['level_name']} | context={signal.get('market_context')} | "
        f"rule={signal.get('strategy_rule')} | gate={signal.get('gate_reason')} — chờ FILLED rồi đặt SL/TP"
    )
    save_state()
    return ACTIVE_TRADE
...
def open_trade(signal, price, cycle_ts):
    if LIVE_TRADE:
        if not live_trading_ready():
            print(
                f"[{cycle_ts}] ⚠️ SOL_LIVE_TRADE=true nhưng thiếu BINANCE_API_KEY hoặc Ed25519 PEM — bỏ qua lệnh"
            )
            return None
...
def current_loss_streak(history):
    streak = 0
    for trade in reversed(history or []):
...
def load_state():
    ...
    persisted_loss_streak = current_loss_streak(TRADE_HISTORY)

    if RESET_LOSS_STREAK_ON_START:
        # Match old behavior: a script restart starts a fresh loss-streak session.
        # Trade history, balance, and active PENDING/OPEN workflow are still restored.
        CONSECUTIVE_LOSSES = 0
        if persisted_loss_streak > 0:
            print(
                f"ℹ️ Loss streak from state={persisted_loss_streak}; "
                "reset to 0 on restart because SOL_RESET_LOSS_STREAK_ON_START=true."
            )
    else:
        # Strict mode: carry loss streak across restarts.
        CONSECUTIVE_LOSSES = persisted_loss_streak
...
def run():
    ...
    if LIVE_TRADE and live_trading_ready():
        ...
        mode_label = "SIM"
    print("=" * 70)
    print(f"SOL HVN/LVN {mode_label} | {SYMBOL} | poll={PRICE_POLL_SEC}s | profile={PROFILE_REFRESH_SEC}s")
    if ENV_FILES_LOADED:
    ...
    if LIVE_TRADE:
        ex = f" ~{calc_order_qty(150)[0]} SOL @150" if ORDER_QTY_FIXED is None else f" fixed_qty={ORDER_QTY_FIXED}"
        margin_note = (
        ...
        print(
            ...
            f"api_key={'set' if API_KEY else 'MISSING — export BINANCE_API_KEY hoặc ./run_sol_live.sh'} | "
            f"pem={'OK' if os.path.isfile(ED25519_KEY_PATH) else 'MISSING'} ({ED25519_KEY_PATH})"
        )
    print(
        f"max_loss_streak={MAX_CONSECUTIVE_LOSSES} | current_loss_streak={CONSECUTIVE_LOSSES} | "
        f"reset_loss_on_start={RESET_LOSS_STREAK_ON_START} | scan_status={SCAN_STATUS_SEC}s | "
    ...
    while True:
        if CONSECUTIVE_LOSSES >= MAX_CONSECUTIVE_LOSSES:
            if RESET_LOSS_STREAK_ON_START:
                print(
                    f"\n⛔ Dừng script: {MAX_CONSECUTIVE_LOSSES} lần thua liên tiếp trong session hiện tại. "
                    "Restart script để reset loss streak và chạy lại như cũ."
                )
            else:
                print(
                    f"\n⛔ Dừng script: {MAX_CONSECUTIVE_LOSSES} lần thua liên tiếp. "
                    "Loss streak đang restore từ state; muốn tiếp tục thì export "
                    "SOL_RESET_LOSS_STREAK_ON_START=true, reset/xóa state, hoặc tăng MAX_CONSECUTIVE_LOSSES."
                )
            save_state()
            break
